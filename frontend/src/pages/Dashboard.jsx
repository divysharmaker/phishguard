  ...
  return (
    <div className={styles.threatMeter}>
      ...
      <div className={styles.threatBar}>
        <div className={styles.threatFill} style={{ width: `${pct}%`, background: color }} />
      </div>
...
function StatsCharts({ totalScans, phishingFound, recentHistory }) {
  ...
  // Build line chart from recent history
  const lineData = (() => {
  ...
  const COLORS = ['#ff2d55', '#00e676', '#ffab00']
          ...
          <div className={`${styles.chartCard} ${styles.chartWide}`}>
            ...
            <ResponsiveContainer width="100%" height={160}>
              <LineChart data={lineData} margin={{ top: 5, right: 15, left: -20, bottom: 5 }}>
                ...
                <Line type="monotone" dataKey="scans" stroke="var(--cyan)" strokeWidth={2} dot={false} name="Scans" />
                <Line type="monotone" dataKey="threats" stroke="var(--red)" strokeWidth={2} dot={false} name="Threats" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
        ...
        {scans.slice(0, 5).map((s, i) => (
          <div key={i} className={styles.recentItem}>
            ...
            <span className={styles.recentTime}>
              {new Date(s.scanned_at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true })}
            </span>
          </div>
        ))}
      </div>
  ...
  return (
    <div className={styles.quickTest}>
      ...
      <div className={styles.quickList}>
        {urls.map((u, i) => (
          <button key={i} className={`${styles.quickBtn} ${styles[u.type]}`}
...
export default function Dashboard() {
  ...
  // Fetch recent scans for activity panel
  useEffect(() => {
  ...
  return (
    <div className={styles.page}>
          ...
          <div className={styles.statRow}>
            ...
            <div className={styles.statCard}>
              <span className={styles.statVal} style={{ color: 'var(--green)' }}>
                {Math.max(0, (user?.total_scans ?? 0) - (user?.phishing_found ?? 0))}
              </span>
              <span className={styles.statLbl}>Safe</span>
            </div>
          </div>
          <LiveClock />
        </div>
      ...
      <StatsCharts
        ...
        phishingFound={user?.phishing_found ?? 0}
        recentHistory={recentScans}
      />
          ...
          {tab === 'single' && (
            <div className="panel">
              ...
              <div className={styles.inputRow}>
                <input className="pg-input" value={url}
                  onChange={e => setUrl(e.target.value)}
                  placeholder="https://example.com/login"
                  onKeyDown={e => e.key === 'Enter' && !loading && scanSingle()} />
                <button className="pg-btn sm" onClick={() => scanSingle()} disabled={loading} style={{ flexShrink: 0, minWidth: 80 }}>
                  {loading ? '...' : 'Scan'}
                </button>
              </div>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <div className="panel" style={{ overflowX: 'auto', padding: 0, marginTop: 10 }}>
                <table className={styles.batchTable}>
                  <thead>
                    <tr>{['URL','Score','Verdict','Flags'].map(h => <th key={h}>{h}</th>)}</tr>
                  </thead>
                  <tbody>
